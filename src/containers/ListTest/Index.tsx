...
const TAB_TYPE: any = ["all", "created", "completed"];
...
const ListTest: React.FC = () => {
  ...
  const [testItemDetail, settestItemDetail] = useState<Test>(
    initiTestItemDetail
  );
      ...
      <Grid container>
        ...
        <GridListTest item xs={12}>
          <Grid container spacing={2}>
            ...
            {!isLoading &&
              testsData?.length > 0 &&
              testsData?.map((test: any, index: number) => (
        ...
        <Grid item xs={12}>
          <ContainerPagin>
            {totalPage > 1 && <PaginationLink totalPage={totalPage} />}
          </ContainerPagin>
        </Grid>
      </Grid>
    </Container>
